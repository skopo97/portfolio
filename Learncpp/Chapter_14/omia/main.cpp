...
public:
  // default constructor
  IntPair(const int x, const int y) : m_x{x}, m_y{y} {}
...
void print(const IntPair& p) { std::cout << "(" << p.x() << ", " << p.y() << ")\n"; }
...
private:
  // These members are private, so they are not accessible from outside the
  // class
  int m_id{};
  std::string m_name{};
